...
def _merge_cicids(
    ...
    merged_filename: str = CICIDS2017.DEFAULT_MERGED_FILENAME,
    force: bool = False,
) -> Path:
    ...
    if merged_path.is_file():
        if not force:
            print(
                f"[!] {merged_path} already exists, use `force=True` to override. Skipping...",
                file=sys.stderr,
            )
            return merged_path
...
def _convert_attempted_to_benign(
    input_path: Path,
    output_path: Path = None,
    label_name=CICIDS2017.LABEL_COLUMN_NAME,
    force: bool = False,
) -> Path:
    ...
    if output_path.is_file():
        if not force:
            print(
                f"[!] {output_path} already exists, use `force=True` to override. Skipping...",
                file=sys.stderr,
            )
            return output_path
...
def _create_subset(
    ...
    if output_path is None:
        output_path = input_path.with_stem(
            f"{input_path.stem}_idx={start_sample}_n={n_samples}"
        )
    ...
    if n_samples is not None and cnt_samples < n_samples:
        print(
            f"[!] subset generation processed {cnt_samples} which is smaller than specified {n_samples}",
            file=sys.stderr,
        )
...
def predict_cicids_filename(
    convert_attempted: bool, start_sample: int, n_samples: int
) -> str:
    """Helper function to make sure dataset path is compliant with the convention"""

    suffix = (
        ""
        if (start_sample == 0 and n_samples is None)
        else f"_idx={start_sample}_n={n_samples}"
    )
