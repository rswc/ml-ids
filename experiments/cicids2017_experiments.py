...
from metrics import MetricWrapper, AttackDetectionRate, FalseDiscoveryRate, FalsePositiveRate
...
if __name__ == '__main__':
    cicids_file = generate_cicids_file(
        dataset_path=DATASET_ROOT, n_samples=N_SAMPLES, 
        start_sample=START_IDX, convert_attempted=CONVERT_ATTEMPTED 
    )

    dataset = CICIDS2017(cicids_file.name, dataset_dir=DATASET_ROOT, n_samples=N_SAMPLES, convert_attempted=CONVERT_ATTEMPTED)
    
    analyzer = DatasetAnalyzer(dataset, WINDOW_SIZE, OUT_DIR, project=PROJECT) 
    # analyzer.analyze()
    
    model = ARFClassifier(drift_detector=NoDrift())
    runner = ExperimentRunner(model, dataset, METRICS, OUT_DIR, project=PROJECT, enable_tracker=False) 
    runner.run()
