...
from cicids.preprocess import (
    _merge_cicids,
    _create_subset,
    _convert_attempted_to_benign,
)
import csv
import pytest
...
class TestCICIDSPreprocess:
    ...
    def test_convert_attempted_to_benign(self, tmp_path):
        ...
        assert (
            conv_dataset.name
            == "all_days_noatt.csv"
            == predict_cicids_filename(True, 0, None)
        )
    ...
    def test_create_subset_converted(self, tmp_path):
        ...
        assert (
            b_only.name
            == "all_days_noatt_idx=50_n=50.csv"
            == predict_cicids_filename(True, 50, 50)
        )
        ...
        assert (
            bc_only.name
            == "all_days_noatt_idx=50_n=None.csv"
            == predict_cicids_filename(True, 50, None)
        )
    ...
    def test_create_subset_attempted(self, tmp_path):
        ...
        assert (
            b_only.name
            == "all_days_idx=50_n=50.csv"
            == predict_cicids_filename(False, 50, 50)
        )
        with open(b_only, "r") as dataset_csv:
            csv_reader = csv.reader(dataset_csv)
            assert next(csv_reader)[0] == "column"
        ...
        assert (
            bc_only.name
            == "all_days_idx=50_n=None.csv"
            == predict_cicids_filename(False, 50, None)
        )
        with open(bc_only, "r") as dataset_csv:
            csv_reader = csv.reader(dataset_csv)
            assert next(csv_reader)[0] == "column"
